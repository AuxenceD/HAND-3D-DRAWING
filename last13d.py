...
class Camera:
    def __init__(self):
        ...
        self.rotation_x = 0
        self.rotation_y = 0
        self.zoom = 0  # Zoom global de la scène
        
        # Distance fixe du plan de projection
        self.projection_distance = 5

    def get_projection_point(self, hand_point):
        """Convertit les coordonnées de la main en coordonnées de dessin"""
        # Point de base à la distance de projection
        base_point = np.array([
            (hand_point.x - 0.5) * 10,   # X
            (hand_point.y - 0.5) * 10,   # Y
            self.projection_distance      # Z toujours à la même distance
        ])
        ...
        rotated_point = np.dot(Ry, np.dot(Rx, base_point))
...
def main():
    try:
        ...
        pygame.display.set_caption("Dessin 3D Plan Fixe")
        ...
        while running:
            ...
            # Translation globale basée sur le zoom
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # Obtenir le point de projection
                    drawing_point = camera.get_projection_point(thumb_tip)
